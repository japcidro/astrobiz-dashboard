 ...
 * This module groups raw orders into buyers and derives the numbers the CEO
 * reads: when they bought, how many units, what SRP they paid, and whether the
 * buying pattern looks like a reseller rather than a consumer.
 */
...
export interface RepeatBuyerOrder {
  ...
  // Days since this buyer's previous live order. null on their first one.
...
export interface RepeatBuyer {
  ...
  orders_count: number; // live orders in the window
  cancelled_count: number; // cancelled / voided / refunded, excluded from money
  total_units: number;
  total_spent: number;
  ...
  avg_days_between: number | null; // null when there is only one live order
  cod_count: number;
 ...
 * live (non-cancelled) orders. Cancelled orders stay attached to the buyer for
 * context but never count toward orders, units or money.
 */
export function buildRepeatBuyers(
  orders: ShopifyOrder[],
  { minOrders, windowDays, now = new Date() }: BuildRepeatBuyersOptions
  ...
  for (const acc of groups.values()) {
    ...
    const live = acc.orders.filter((o) => !o.is_dead);
    if (live.length === 0) continue; // nothing but cancelled orders — not a buyer

    let previousIso: string | null = null;
    const gaps: number[] = [];
    for (const order of acc.orders) {
      if (order.is_dead) continue;
    ...
    if (live.length >= RESELLER_MIN_ORDERS) {
      reasons.push(`${live.length} orders in window`);
    }
    ...
    allBuyers.push({
      ...
      cancelled_count: acc.orders.length - live.length,
